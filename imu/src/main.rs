...
fn main() -> io::Result<()> {
    ...
    mpu9250.accel_data_rate(AccelDataRate::DlpfConf(Dlpf::_6)).expect("Err setting rate");
    ...
    const IMU_SAMPLE_SEC: f64 = 0.004;
            ...
            match mpu9250.all::<[f32; 3]>() {
                Ok(all ) => {
                    ...
                    loop {
                        ...
                        let tt = t.elapsed();
                        match Duration::from_micros((IMU_SAMPLE_SEC * (1000000.0 - 7000.0)) as u64).checked_sub(t.elapsed()) {
                            Some(diff) => {
                                thread::sleep(diff);
                                loop_time = tt;
                                break;
